...
router.post("/productdata",isLoggedin,uploads.single("image"), async function(req,res,next){
 try {
...
router.get("/delete/:id",async function(req,res,next){
  try {
    ...
    fs.unlinkSync(path.join(__dirname,"../","public","images",deleteproduct.image))
...
router.post("/editdata/:id",isLoggedin,uploads.single("image") ,async function(req,res,next){
  try {
  // const post = await productSchema.findById(req.params.id);
  // await post.save();
  const editdata=await productSchema.findByIdAndUpdate(req.params.id,req.body);
  if(req.file.filename){
      fs.unlinkSync(path.join(__dirname,"../","public","images",editdata.image))
      }
